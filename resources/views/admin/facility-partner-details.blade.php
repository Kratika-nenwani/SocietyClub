...
@section('content')
    <main class="main__content_wrapper">
        <div style="padding: 20px;">
            <h2 style="font-size: 24px; font-weight: bold; color: #333;">Facility Partner Details</h2>
            <br>
                <p><strong>Name:</strong> {{ $user->name }}</p>
                <p><strong>Email:</strong> {{ $user->email }}</p>
                <p><strong>Phone Number:</strong> {{ $user->phone }}</p>
                <p><strong>Facility:</strong> {{ $user->service }}</p>
                <p><strong>Description:</strong> {{ $user->description }}</p>
                <p><strong>Society Name:</strong> {{ $user->society_name }}</p>
                <a href="{{ route('facility-partner') }}" style="padding: 8px 12px; background-color: #2196F3; color: white; text-decoration: none; border-radius: 4px;">Back to List</a>
        </div>
    </main>
@endsection
@section('jscontent')
